...
PHRASES = {
    ...
    "Поддерживаются Bitunix, Binance, Bybit, Bitget и Gate.": "Supported exchanges: Bitunix, Binance, Bybit, Bitget and Gate.",
    "Команды:": "Commands:",
    ...
    "Не удалось получить ставки ни с одной биржи. Попробуйте позже.": "Could not retrieve rates from any exchange. Try again later.",
    "Настройки FVG 15м": "FVG 15m settings",
    "Модуль:": "Module:",
    "Подтверждённые:": "Confirmed:",
...
def _translate_text(text: str) -> str:
    translated = text
    for exchange in ("Bitunix", "Binance", "Bybit", "Bitget", "Gate"):
...
def _compact_fvg(text: str, language: str) -> str:
    ...
    if language == "en":
        state = "Preliminary" if preliminary else "Confirmed"
        return f"{icon} FVG {symbol} · {timeframe}\nZone: {zone} · Price: {signal}\n{state}"
    state = "Предварительный" if preliminary else "Подтверждённый"
...
def localize_text(text: str, language: str, mode: str = "detailed") -> str:
    ...
    if mode == "compact" and is_funding_alert:
