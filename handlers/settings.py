...
from handlers.fvg_dashboard import show_fvg_dashboard
from handlers.menu import build_main_menu, build_reply_menu
...
def settings_keyboard(chat_id: int, preferences=None) -> InlineKeyboardMarkup:
    ...
    if language == "en":
        rows = [
            ...
            [InlineKeyboardButton("📉 FVG center", callback_data="settings:fvg")],
            [InlineKeyboardButton("🔔 Alert center", callback_data="settings:alerts")],
        ]
        ...
        rows = [
            ...
            [InlineKeyboardButton("📉 FVG-центр", callback_data="settings:fvg")],
            [InlineKeyboardButton("🔔 Центр уведомлений", callback_data="settings:alerts")],
        ]
...
def build_alert_center_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📉 FVG-сигналы", callback_data="fvg-ui:signals")],
        [InlineKeyboardButton(
            "💸 Уведомления о фандинге",
            callback_data="funding-alert:open",
        )],
        [InlineKeyboardButton("⬅️ Главное меню", callback_data="settings:main")],
    ])


async def show_alert_center(message, *, edit=False) -> None:
    ...
    await method(
        ...
        reply_markup=build_alert_center_menu(),
        parse_mode="HTML",
    )
...
async def persistent_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "fvg":
        await show_fvg_dashboard(message, chat_id, edit=False)
    elif action == "funding":
        await show_funding(message, context, edit=False)
    elif action == "alerts":
        await show_alert_center(message, edit=False)
    elif action == "stats":
...
async def settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action.startswith("language:"):
        ...
        await query.answer(
            "Язык изменён." if language == "ru" else "Language updated."
        )
        await show_settings(query.message, chat_id, edit=True)
        await query.message.reply_text(
            "Нижнее меню обновлено."
            if language == "ru"
            else "Bottom menu updated.",
            reply_markup=build_reply_menu(),
        )
        return
    if action.startswith("mode:"):
        preferences = PREFERENCES.set_message_mode(
            chat_id,
            action.split(":", 1)[1],
        )
        await query.answer(
    ...
    if action in {"open", "back"}:
        ...
        await show_alert_center(query.message, edit=True)
    elif action == "funding":
