...
async def _preferences(chat_id: int) -> dict:
    return await asyncio.to_thread(PREFERENCES.user, chat_id)
...
async def show_settings(message, chat_id: int, *, edit=False) -> None:
    preferences = await _preferences(chat_id)
...
async def persistent_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "fvg":
        await message.reply_text(
            "Settings are saved separately for your Telegram ID."
            if english
            else "Настройки сохраняются отдельно для вашего Telegram ID.",
...
async def settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action.startswith("language:"):
        ...
        await asyncio.to_thread(PREFERENCES.set_language, chat_id, language)
