...
The module is intentionally free of Telegram, network and persistence calls.  It
accepts one closed market candle and returns a new immutable zone state plus
idempotent domain events.  This keeps lifecycle behaviour testable and allows
the entire feature to be disabled without changing the existing alert path.
"""
...
class FvgLifecycleConfig:
    ...
    def __post_init__(self) -> None:
        if not self.approaching_zone_widths.is_finite() or self.approaching_zone_widths <= 0:
        ...
        if any(value <= 0 or value >= 100 for value in self.fill_thresholds):
            raise ValueError("fill thresholds must be between 0 and 100")
...
class FvgZoneEvent:
    ...
    def dedupe_key(self, fvg_id: str) -> str:
        if self.event_type is FvgLifecycleEventType.RETOUCH:
...
def _fill_percent(candle: Candle, zone: FvgZoneState) -> Decimal:
    if zone.direction is FvgDirection.BULLISH:
        penetration = zone.upper_bound - candle.low
    else:
        penetration = candle.high - zone.lower_bound
...
def _distance(candle: Candle, zone: FvgZoneState) -> Decimal:
    price = candle.close
    if price > zone.upper_bound:
        return price - zone.upper_bound
    if price < zone.lower_bound:
        return zone.lower_bound - price
    return ZERO


def _expected_entry_relation(zone: FvgZoneState) -> ZoneRelation:
    return ZoneRelation.ABOVE if zone.direction is FvgDirection.BULLISH else ZoneRelation.BELOW


def _touch_price(zone: FvgZoneState) -> Decimal:
    # OHLC candles do not expose the exact intrabar touch price.  Persist the
    # near boundary as a deterministic estimate and keep the source in payload.
    return zone.upper_bound if zone.direction is FvgDirection.BULLISH else zone.lower_bound
...
def advance_zone(
    ...
    """Advance a zone once for one closed candle.

    The function is idempotent by ``last_processed_candle`` and emits every
    crossed fill threshold once through ``fill_threshold_mask``.  When one
    candle crosses the entire zone and closes beyond the far edge, the maximum
    fill remains 100% but the final status is INVALIDATED.
    """
    config = config or FvgLifecycleConfig()
    if zone.is_terminal:
        return FvgLifecycleTransition(zone, (), False)
    if candle.symbol.upper() != zone.symbol or not candle.is_closed or not candle.is_complete:
        return FvgLifecycleTransition(zone, (), False)
    ...
    elif overlaps and zone.last_relation is expected_entry:
        touch_count += 1
        events.append(
            FvgZoneEvent(
    ...
    if zone.direction is FvgDirection.BULLISH:
        invalidated = candle.close <= zone.lower_bound - buffer_value
    else:
        invalidated = candle.close >= zone.upper_bound + buffer_value
    ...
    if invalidated:
        ...
        near = _distance(candle, zone) <= zone.zone_size * config.approaching_zone_widths
        if near:
            if zone.status is FvgLifecycleStatus.DETECTED:
                ...
                events.append(
                    FvgZoneEvent(
                        ...
                        candle_time=candle.open_time,
                        payload={
                            "distance": str(_distance(candle, zone)),
    ...
    processed_bars = zone.processed_bars + 1
