...
- публичный или приватный режим для подтверждённого администратора;
...
- Python 3.12;
- aiohttp;
- существующие `UserPreferences`, `FvgAlertSettings`, `FundingAlertStore`, `FundingExchangeStore`, `RuntimeSettings` и `AccessRegistry`.
...
Также новые тесты автоматически входят в общий запуск:
...
1. Выполнить frontend typecheck и production build в CI.
2. Разместить frontend и API за HTTPS.
3. Зарегистрировать URL в BotFather.
4. Добавить тестовую кнопку открытия только для администратора.
5. Провести параллельный тест с текущим Telegram UI.
6. Реализовать отдельные подтверждаемые admin endpoint для backup, restart и allowlist.
7. После полного принятия переключить настройки на Mini App и удалить только ставшие лишними элементы бота.
