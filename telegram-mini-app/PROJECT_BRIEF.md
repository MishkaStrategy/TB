...
- Frontend находится только в `telegram-mini-app/`.
...
- состояния загрузки, ошибки, подтверждение сохранения;
...
6. Read-only диагностика администратора.
7. Защищённые административные действия.
8. HTTPS-размещение и регистрация URL в BotFather.
9. Тестовая кнопка только для администратора.
10. Параллельный тест со старым Telegram UI.
11. Финальная интеграция только после отдельного подтверждения.
...
- честно перечислять запущенные и незапущенные проверки;
- не объединять PR без отдельного разрешения.

## Критерий завершения

Mini App считается готовым только когда все настройки читаются и сохраняются через production backend, серверная авторизация и роли проверены, интерфейс протестирован в Telegram-клиентах, автоматические тесты проходят, документация развёртывания готова и владелец проекта отдельно разрешил финальную интеграцию и удаление лишних настроек из бота.
