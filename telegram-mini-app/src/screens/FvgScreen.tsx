...
function ScopeSelector({ scope, language, onChange }: { scope: FilterScope; language: Language; onChange: (scope: FilterScope) => void }) {
  const rows: Array<[keyof FilterScope, string, string]> = [
    ["confirmedFvg", "Confirmed FVG", "Confirmed FVG"],
...
export function FvgScreen({
  ...
  return <div className="screen-stack fvg-screen">
    <PageHeader eyebrow="FVG" title="Fair Value Gap" description={tx(language, "Exchange-aware инструменты, таймфреймы и фильтры подтверждённых FVG.", "Exchange-aware instruments, timeframes and confirmed FVG filters.")} trailing={<Toggle checked={enabled} onChange={onToggleEnabled} label={tx(language, "Включить FVG", "Enable FVG")} />} />
    ...
    <Section title={tx(language, "Инструменты", "Instruments")} subtitle={tx(language, "Одинаковый symbol на разных биржах считается отдельным инструментом", "The same symbol on different exchanges is a separate instrument")}>
      {instruments.length ? <div className="instrument-list">{instruments.map((item) => {
        ...
        return <button type="button" key={item.key} className={`instrument-card ${selected?.key === item.key ? "selected" : ""}`} onClick={() => onSelect(item.key)}>
          <div className="instrument-card-copy"><div><strong>{item.symbol}</strong><span>{exchangeLabels[item.exchange]}</span></div><div className="timeframe-line">{item.timeframes.map((tf) => <span key={tf}>{tf}</span>)}</div><small>{item.enabled ? "Active" : "Paused"}{filters ? ` · ${filters} ${tx(language, "filters active", "filters active")}` : ""}</small></div>
      ...
      })}</div> : <EmptyState title={tx(language, "У вас пока нет инструментов", "No instruments yet")} description={tx(language, "Добавьте первую пару exchange + symbol.", "Add your first exchange + symbol pair.")} />}
    </Section>

    <Section title={tx(language, "Добавить инструмент", "Add instrument")} subtitle={atLimit ? tx(language, `Достигнут технический лимит: ${maxSymbols}.`, `Technical limit reached: ${maxSymbols}.`) : tx(language, "Новый инструмент стартует с таймфреймом 15m", "New instruments start with the 15m timeframe")} className="add-instrument-panel">
      <div className="add-instrument-form"><label><span>{tx(language, "Биржа", "Exchange")}</span><select value={newExchange} onChange={(event) => setNewExchange(event.target.value as Exchange)} disabled={atLimit}>{exchangeOrder.map((exchange) => <option key={exchange} value={exchange}>{exchangeLabels[exchange]}</option>)}</select></label><label className="symbol-field"><span>Symbol</span><input value={newSymbol} disabled={atLimit} placeholder="BTCUSDT" autoCapitalize="characters" onChange={(event) => setNewSymbol(event.target.value.toUpperCase())} onKeyDown={(event) => { if (event.key === "Enter") submitAdd(); }} /></label><button type="button" className="primary-button" disabled={atLimit} onClick={submitAdd}><Icon name="plus" size={18} />{tx(language, "Добавить", "Add")}</button></div>
    </Section>

    {selected ? <Section title={`${selected.symbol} · ${exchangeLabels[selected.exchange]}`} subtitle={tx(language, "Конфигурация инструмента", "Instrument configuration")} action={<Toggle checked={selected.enabled} onChange={(value) => onToggleInstrument(selected.key, value)} label={tx(language, "Включить инструмент", "Enable instrument")} />} className="instrument-editor">
      <div className="editor-grid">
        ...
        <label className="field"><span>Symbol</span><input value={selected.symbol} readOnly aria-readonly="true" /></label>
      </div>
      ...
      <div className="editor-block"><div className="editor-label"><strong>{tx(language, "Направления и подтверждение", "Directions and confirmation")}</strong><span>{tx(language, "Общие для FVG-модуля", "Applies to the FVG module")}</span></div><div className="chip-row"><Chip active={bullishEnabled} tone="positive" onClick={() => onToggleBullish(!bullishEnabled)}>Bullish</Chip><Chip active={bearishEnabled} tone="negative" onClick={() => onToggleBearish(!bearishEnabled)}>Bearish</Chip><Chip active={notifyConfirmedFvg} onClick={() => onToggleConfirmed(!notifyConfirmedFvg)}>Confirmed FVG</Chip></div></div>

      <div className="filter-card"><div className="filter-card-head"><div><span className="filter-icon"><Icon name="filter" size={17} /></span><div><strong>{tx(language, "Price filter", "Price filter")}</strong><small>{tx(language, "Диапазон цены и область применения", "Price range and application scope")}</small></div></div><Toggle checked={selected.priceFilter.enabled} onChange={(value) => onUpdatePriceFilter(selected, { enabled: value })} label="Price filter" /></div><div className="filter-fields"><label><span>Min</span><input inputMode="decimal" value={selected.priceFilter.min ?? ""} onChange={(event) => onUpdatePriceFilter(selected, { min: event.target.value || null })} placeholder="0" /></label><label><span>Max</span><input inputMode="decimal" value={selected.priceFilter.max ?? ""} onChange={(event) => onUpdatePriceFilter(selected, { max: event.target.value || null })} placeholder="—" /></label></div><ScopeSelector scope={selected.priceFilter.scope} language={language} onChange={(scope) => onUpdatePriceFilter(selected, { scope })} /></div>

      <div className="filter-card"><div className="filter-card-head"><div><span className="filter-icon"><Icon name="filter" size={17} /></span><div><strong>{tx(language, "FVG size filter", "FVG size filter")}</strong><small>{tx(language, "Минимальный размер FVG", "Minimum FVG size")}</small></div></div><Toggle checked={selected.sizeFilter.enabled} onChange={(value) => onUpdateSizeFilter(selected, { enabled: value })} label="FVG size filter" /></div><div className="filter-fields"><label><span>Min</span><input inputMode="decimal" value={selected.sizeFilter.min ?? ""} onChange={(event) => onUpdateSizeFilter(selected, { min: event.target.value || null })} placeholder="0" /></label><label><span>{tx(language, "Единица", "Unit")}</span><select value={selected.sizeFilter.unit} onChange={(event) => onUpdateSizeFilter(selected, { unit: event.target.value as FvgSizeUnit })}><option value="USD">USD</option><option value="PERCENT">%</option></select></label></div><ScopeSelector scope={selected.sizeFilter.scope} language={language} onChange={(scope) => onUpdateSizeFilter(selected, { scope })} /></div>
