...
The confirmed cause was production environment contamination: a stale `MAX_SYMBOLS_PER_USER=20` override affected candidate unit tests. Releases `1.3.1` through `1.3.3` isolate candidate tests with `env -i`, retain a complete test log and cap the effective FVG instrument limit at 10.

`v1.3.3` is the current audited deployment target. Telegram Mini App PR #53 is excluded.
