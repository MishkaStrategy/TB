...
FVG-база наблюдает в том числе собственные observability-таблицы. Это намеренно: следующий snapshot показывает полную фактическую стоимость хранения метрик. Target read-соединение закрывается до записи snapshot, поэтому collector не удерживает read transaction во время изменения FVG-базы.
