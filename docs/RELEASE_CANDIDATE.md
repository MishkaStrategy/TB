# Проверка release candidate 1.0.0-rc3
...
Регрессионный тест установщика проверяет, что pre-switch backup запускается до
атомарного переключения релиза, но использует Python и код staging-каталога.
Это покрывает чистую первую установку, когда `/opt/fvg-alert-bot` ещё отсутствует.
...
2. Установить rc3 на Ubuntu 22.04/24.04 или Debian 12.
