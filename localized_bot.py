...
def _localize_safely(text: str, language: str, mode: str) -> str:
    ...
    protected = re.sub(r"\bиз\b", "__RU_OF__", text).replace("из", "__RU_IZ__")
...
class LocalizedExtBot(ExtBot):
    ...
    async def send_message(self, chat_id, text, *args, **kwargs):
        ...
        kwargs["reply_markup"] = self._localize_markup(kwargs.get("reply_markup"), preferences["language"])
    ...
    async def answer_callback_query(self, callback_query_id, *args, text=None, **kwargs):
        if text is not None:
            ...
            text = _localize_safely(text, preferences["language"], preferences["message_mode"])
        return await super().answer_callback_query(callback_query_id, *args, text=text, **kwargs)
