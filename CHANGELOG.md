...
- значение `0.01514` теперь отображается как `1.5140%`, а конфликтующее ticker-значение `-1.514051` не превращается в `−151.4051%`;
- добавлен регрессионный тест, воспроизводящий фактическую ошибку production-интерфейса.
...
- ставка Bitunix интерпретируется как доля: `0.0005` отображается как `0.05%`;
