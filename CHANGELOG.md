...
- публикация immutable release assets больше не зависит от отсутствующего `gh` CLI на self-hosted runner;
- архив и SHA-256 загружаются через аутентифицированный GitHub API action с сохранением идемпотентной проверки существующих assets;
- обычный неизвестный или уже существующий tag по-прежнему не перезаписывается.
